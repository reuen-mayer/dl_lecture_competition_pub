...
class VQAModel(nn.Module):
    ...
    def __init__(self, n_answer: int):#BERT用
        super().__init__()
        self.resnet = ResNet101()#ResNet101を使用
        ...
        self.fc = nn.Sequential(
            nn.Linear(1024, 512),#非BERTではこちらのサイズ
...
def eval(model, dataloader, optimizer, criterion, device):
    ...
    start = time.time()
#    for image, question, answers, mode_answer in dataloader:#非BERT使用時はこちらを使用
#        image, question, answer, mode_answer = \
#            image.to(device), question.to(device), answers.to(device), mode_answer.to(device)
    with torch.no_grad():#BERT使用時はこちらを使用
        for image, question, answers, mode_answer in dataloader:
            image, answers, mode_answer = \
                image.to(device, non_blocking=True), answers.to(device, non_blocking=True), mode_answer.to(device, non_blocking=True)
            with autocast():#autocastの使用
